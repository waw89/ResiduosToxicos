...
public class SolicitudTrasladoModelJpaController implements Serializable {
    ...
    public void edit(SolicitudTrasladoModel solicitudTrasladoModel) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            List<TransportistaModel> transOld = persistentSolicitudTrasladoModel.getTrans();
            List<TransportistaModel> transNew = solicitudTrasladoModel.getTrans();
            ...
            List<TransportistaModel> attachedTransNew = new ArrayList<TransportistaModel>();
            for (TransportistaModel transNewTransportistaModelToAttach : transNew) {
                transNewTransportistaModelToAttach = em.getReference(transNewTransportistaModelToAttach.getClass(), transNewTransportistaModelToAttach.getId());
                attachedTransNew.add(transNewTransportistaModelToAttach);
            }
            transNew = attachedTransNew;
            solicitudTrasladoModel.setTrans(transNew);
            if (prodNew != null) {
            ...
            for (TransportistaModel transOldTransportistaModel : transOld) {
                if (!transNew.contains(transOldTransportistaModel)) {
                    transOldTransportistaModel.getListaSolicitudes().remove(solicitudTrasladoModel);
                    transOldTransportistaModel = em.merge(transOldTransportistaModel);
                }
            }
            for (TransportistaModel transNewTransportistaModel : transNew) {
                if (!transOld.contains(transNewTransportistaModel)) {
                    transNewTransportistaModel.getListaSolicitudes().add(solicitudTrasladoModel);
                    transNewTransportistaModel = em.merge(transNewTransportistaModel);
                }
            }
    ...
    public void destroy(Long id) throws NonexistentEntityException {
        ...
        try {
            ...
            List<TransportistaModel> trans = solicitudTrasladoModel.getTrans();
            for (TransportistaModel transTransportistaModel : trans) {
                transTransportistaModel.getListaSolicitudes().remove(solicitudTrasladoModel);
                transTransportistaModel = em.merge(transTransportistaModel);
            }
            ProductorModel prod = solicitudTrasladoModel.getProd();
            if (prod != null) {
